...
func NewEuler[T Float](x, y, z T, order EulerOrder) *Euler[T] {
	return &Euler[T]{
		X:     x,
		Y:     y,
		Z:     z,
...
func (e *Euler[T]) SetFromRotationMatrix(m Matrix4[T], order EulerOrder, update bool) *Euler[T] {
	...
	case EULER_ORDER_XYZ:
		e.Y = T(stdmath.Asin(Clamp(m13, -1, 1)))

		if stdmath.Abs(m13) < 0.9999999 {
			e.X = T(stdmath.Atan2(-m23, m33))
			e.Z = T(stdmath.Atan2(-m12, m11))
		} else {
			e.X = T(stdmath.Atan2(m32, m22))
			e.Z = 0
		}
	case EULER_ORDER_YXZ:
		e.X = T(stdmath.Asin(-Clamp(m23, -1, 1)))

		if stdmath.Abs(m23) < 0.9999999 {
			e.Y = T(stdmath.Atan2(m13, m33))
			e.Z = T(stdmath.Atan2(m21, m22))
		} else {
			e.Y = T(stdmath.Atan2(-m31, m11))
			e.Z = 0
		}
	case EULER_ORDER_ZXY:
		e.X = T(stdmath.Asin(Clamp(m32, -1, 1)))

		if stdmath.Abs(m32) < 0.9999999 {
			e.Y = T(stdmath.Atan2(-m31, m33))
			e.Z = T(stdmath.Atan2(-m12, m22))
		} else {
			e.Y = 0
			e.Z = T(stdmath.Atan2(m21, m11))
		}
	case EULER_ORDER_ZYX:
		e.Y = T(stdmath.Asin(-Clamp(m31, -1, 1)))

		if stdmath.Abs(m31) < 0.9999999 {
			e.X = T(stdmath.Atan2(m32, m33))
			e.Z = T(stdmath.Atan2(m21, m11))
		} else {
			e.X = 0
			e.Z = T(stdmath.Atan2(-m12, m22))
	...
	case EULER_ORDER_YZX:
		e.Z = T(stdmath.Asin(Clamp(m21, -1, 1)))

		if stdmath.Abs(m21) < 0.9999999 {
			e.X = T(stdmath.Atan2(-m23, m22))
			e.Y = T(stdmath.Atan2(-m31, m11))
		} else {
			e.X = 0
			e.Y = T(stdmath.Atan2(m13, m33))
	...
	case EULER_ORDER_XZY:
		e.Z = T(stdmath.Asin(-Clamp(m12, -1, 1)))

		if stdmath.Abs(m12) < 0.9999999 {
			e.X = T(stdmath.Atan2(m32, m22))
			e.Y = T(stdmath.Atan2(m13, m11))
		} else {
			e.X = T(stdmath.Atan2(-m23, m33))
			e.Y = 0
		}
