...
type Vector2[T Float] struct {
	X T
	Y T
}

func NewVector2[T Float](x, y T) *Vector2[T] {
	return &Vector2[T]{X: x, Y: y}
}

func NewZeroVector2[T Float]() *Vector2[T] {
	return NewVector2[T](0, 0)
}

func NewOneVector2[T Float]() *Vector2[T] {
...
func AddVector2s[T Float](a, b Vector2[T]) *Vector2[T] {
	return NewVector2(a.X+b.X, a.Y+b.Y)
}

func AddScaledVector2[T Float](vector Vector2[T], scalar T) *Vector2[T] {
...
func SubVector2s[T Float](a, b Vector2[T]) *Vector2[T] {
	return NewVector2(a.X-b.X, a.Y-b.Y)
}
...
func (v *Vector2[T]) Length() T {
	return T(stdmath.Sqrt(float64(v.X*v.X + v.Y*v.Y)))
}

func (v *Vector2[T]) ManhattanLength() T {
	return T(stdmath.Abs(float64(v.X) + stdmath.Abs(float64(v.Y))))
}
...
func (v *Vector2[T]) Angle() T {
	angle := stdmath.Atan2(float64(-v.Y), float64(-v.X)) + stdmath.Pi
	return T(angle)
}

func (v *Vector2[T]) AngleTo(vector Vector2[T]) T {
	denominator := stdmath.Sqrt(float64(v.LengthSq() * vector.LengthSq()))
	if denominator == 0 {
		return stdmath.Pi / 2
	}
	theta := float64(v.Dot(vector)) / denominator
	return T(stdmath.Acos(Clamp(theta, -1, 1)))
}

func (v *Vector2[T]) DistanceTo(vector Vector2[T]) T {
	return T(stdmath.Sqrt(float64(v.DistanceToSquared(vector))))
}
...
func (v *Vector2[T]) ManhattanDistanceTo(vector Vector2[T]) T {
	return T(stdmath.Abs(float64(v.X-vector.X)) + stdmath.Abs(float64(v.Y-vector.Y)))
}
...
func LerpVectors[T Float](v1, v2 Vector2[T], alpha T) *Vector2[T] {
	v := NewVector2(
...
func (v *Vector2[T]) RotateAround(center Vector2[T], angle T) *Vector2[T] {
	c := T(stdmath.Cos(float64(angle)))
	s := T(stdmath.Sin(float64(angle)))
	x := v.X - center.X
	y := v.Y - center.Y
