...
type Spherical[T Float] struct {
	Radius, Phi, Theta T
}

func NewSpherical[T Float](radius, phi, theta T) *Spherical[T] {
...
func (s *Spherical[T]) MakeSafe() *Spherical[T] {
	s.Phi = max(EPSILON, min(stdmath.Pi-EPSILON, s.Phi))
	return s
}
...
func (s *Spherical[T]) SetFromCartesianCoords(x, y, z T) *Spherical[T] {
	s.Radius = T(stdmath.Sqrt(float64(x*x + y*y + z*z)))
	if s.Radius == 0 {
		s.Theta = 0
		s.Phi = 0
	} else {
		s.Theta = T(stdmath.Atan2(float64(x), float64(z)))
		s.Phi = T(stdmath.Acos(float64(Clamp(y/s.Radius, -1, 1))))
	}
	return s
}
