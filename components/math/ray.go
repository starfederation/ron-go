...
func NewRay[T Float](origin, dir Vector3[T]) *Ray[T] {
...
func (r *Ray[T]) DistanceToPoint(point Vector3[T]) T {
	return T(stdmath.Sqrt(float64(r.DistanceSqToPoint(point))))
}
...
func (r *Ray[T]) DistanceSqToSegment(v0, v1 Vector3[T], optionalPointOnRay, optionalPointOnSegment *Vector3[T]) T {
	...
	det := stdmath.Abs(1 - float64(a01*a01))
...
func (r *Ray[T]) IntersectSphere(sphere *Sphere[T], target *Vector3[T]) *Vector3[T] {
	...
	thc := T(stdmath.Sqrt(float64(radius2 - d2)))
	t0 := tca - thc
	t1 := tca + thc
	if t1 < 0 {
		return nil
...
func (r *Ray[T]) IntersectBox(box *Box3[T]) *Vector3[T] {
	...
	if tymin > tmin || stdmath.IsNaN(float64(tmin)) {
		tmin = tymin
	}
	if tymax < tmax || stdmath.IsNaN(float64(tmax)) {
		tmax = tymax
	}
	if invdirz >= 0 {
	...
	if tzmin > tmin || stdmath.IsNaN(float64(tmin)) {
		tmin = tzmin
	}
	if tzmax < tmax || stdmath.IsNaN(float64(tmax)) {
		tmax = tzmax
	}
	if tmax < 0 {
		return nil
