...
type CatmullRomCurve3[T Float] struct {
	baseCurve[T]
	Points    []*Vector3[T]
	Closed    bool
	CurveType CurveType
...
func NewCatmullRomCurve3[T Float](points []*Vector3[T], closed bool, curveType CurveType, tension T) *CatmullRomCurve3[T] {
...
func (c *CatmullRomCurve3[T]) GetPoint(t T, optionalTarget *Vector3[T]) *Vector3[T] {
	...
	if c.Closed {
		if intPoint <= 0 {
			intPoint += (int(stdmath.Abs(float64(intPoint))/float64(l)) + 1) * l
		}
	...
	if c.CurveType == CurveTypeCentripetal || c.CurveType == CurveTypeChordal {
		...
		dt0 := T(stdmath.Pow(float64(p0.DistanceToSquared(*p1)), pow))
		dt1 := T(stdmath.Pow(float64(p1.DistanceToSquared(*p2)), pow))
		dt2 := T(stdmath.Pow(float64(p2.DistanceToSquared(*p3)), pow))
