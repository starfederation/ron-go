...
func NewSphere[T Float](center Vector3[T], radius T) *Sphere[T] {
	return &Sphere[T]{
		Center: center,
		Radius: radius,
	}
...
func (s *Sphere[T]) SetFromPoints(points []Vector3[T], optionalCenter *Vector3[T]) *Sphere[T] {
	...
	s.Radius = T(stdmath.Sqrt(float64(maxRadiusSq)))
	return s
}
...
func (s *Sphere[T]) IntersectsPlane(plane Plane[T]) bool {
	return T(stdmath.Abs(float64(plane.DistanceToPoint(s.Center)))) <= s.Radius
}
...
func (s *Sphere[T]) ExpandByPoint(point Vector3[T]) *Sphere[T] {
	...
	if lengthSq > (s.Radius * s.Radius) {
		length := T(stdmath.Sqrt(float64(lengthSq)))
