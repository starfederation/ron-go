...
type Quadtree[T Float] struct {
	bound Box2[T]
	root  *node[T]
}

type Vector2er[T Float] interface {
	Vector2() Vector2[T]
}

// A FilterFunc is a function that filters the points to search for.
type FilterFunc[T Float] func(p Vector2er[T]) bool
...
func New[T Float](bound Box2[T]) *Quadtree[T] {
...
func (q *Quadtree[T]) Remove(p Vector2er[T], eq FilterFunc[T]) bool {
	...
	b := q.bound
	minDistSquared := stdmath.MaxFloat64
	v := &findVisitor[T]{
		point:          p.Vector2(),
		filter:         eq,
		closestBound:   &b,
...
func removeNode[T Float](n *node[T]) bool {
	i := -1
	if n.Children[0] != nil {
		i = 0
...
func (q *Quadtree[T]) KNearestMatching(buf []Vector2er[T], p Vector2[T], k int, f FilterFunc[T], maxDistance ...T) []Vector2er[T] {
	...
	b := q.bound
	maxDistSquared := stdmath.MaxFloat64
	v := &nearestVisitor[T]{
		point:          p,
		filter:         f,
		k:              k,
...
type visitor[T Float] interface {
...
type visit[T Float] struct {
	visitor visitor[T]
}

func newVisit[T Float](v visitor[T]) *visit[T] {
	return &visit[T]{
		visitor: v,
	}
}
...
type findVisitor[T Float] struct {
	point          Vector2[T]
	filter         FilterFunc[T]
	closest        *node[T]
	closestBound   *Box2[T]
...
type nearestVisitor[T Float] struct {
	point          Vector2[T]
	filter         FilterFunc[T]
	k              int
	maxHeap        maxHeap[T]
...
func (v *nearestVisitor[T]) Visit(n *node[T]) {
	...
	if d := point.DistanceToSquared(v.point); d < v.maxDistSquared {
		...
		if len(v.maxHeap) > v.k {
			...
			d = T(stdmath.Sqrt(float64(top.distance)))
			v.closestBound.Min.X = v.point.X - d
			v.closestBound.Max.X = v.point.X + d
			v.closestBound.Min.Y = v.point.Y - d
			v.closestBound.Max.Y = v.point.Y + d
...
type inBoundVisitor[T Float] struct {
	bound    *Box2[T]
...
func childIndex[T Float](cx, cy T, point Vector2[T]) int {
	i := 0
	if point.Y <= cy {
		i = 2
	}
