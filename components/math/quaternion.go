...
func NewQuaternion[T Float](x, y, z, w T) *Quaternion[T] {
	return &Quaternion[T]{X: x, Y: y, Z: z, W: w}
}

func NewIdentityQuaternion[T Float]() *Quaternion[T] {
...
func (q *Quaternion[T]) SlerpFlat(dst []T, dstOffset int, src0 []T, srcOffset0 int, src1 []T, srcOffset1 int, t T) {
	...
	if w0 != w1 || x0 != x1 || y0 != y1 || z0 != z1 {
		...
		sqrSin := 1 - cos*cos
		if sqrSin > T(EPSILON) {
			sin := stdmath.Sqrt(float64(sqrSin))
			len := stdmath.Atan2(sin, float64(cos*T(dir)))
			s = T(stdmath.Sin(float64(s)*len) / sin)
			t = T(stdmath.Sin(float64(t)*len) / sin)
		}
		tDir := t * T(dir)
		x0 = x0*s + x1*tDir
		y0 = y0*s + y1*tDir
		...
		w0 = w0*s + w1*tDir
		if s == 1-t {
			f := T(1 / stdmath.Sqrt(float64(x0*x0+y0*y0+z0*z0+w0*w0)))
			x0 *= f
			y0 *= f
			z0 *= f
			w0 *= f
...
func (q *Quaternion[T]) SetFromEuler(euler Euler[T]) *Quaternion[T] {
	...
	c1 := T(stdmath.Cos(float64(x / 2)))
	c2 := T(stdmath.Cos(float64(y / 2)))
	c3 := T(stdmath.Cos(float64(z / 2)))
	s1 := T(stdmath.Sin(float64(x / 2)))
	s2 := T(stdmath.Sin(float64(y / 2)))
	s3 := T(stdmath.Sin(float64(z / 2)))
	switch order {
...
func (q *Quaternion[T]) SetFromAxisAngle(axis Vector3[T], angle T) *Quaternion[T] {
	...
	s := T(stdmath.Sin(halfAngle))
	q.X = T(axis.X * s)
	q.Y = T(axis.Y * s)
	q.Z = T(axis.Z * s)
	q.W = T(stdmath.Cos(halfAngle))
	return q
}
...
func (q *Quaternion[T]) SetFromRotationMatrix(m Matrix4[T]) *Quaternion[T] {
	...
	if m11 > m22 && m11 > m33 {
		s = T(2 * stdmath.Sqrt(1+m11-m22-m33))
		q.W = T(m32-m23) / s
		q.X = 0.25 * s
		q.Y = T(m12+m21) / s
		q.Z = T(m13+m31) / s
	...
	if m22 > m33 {
		s = T(2 * stdmath.Sqrt(1+m22-m11-m33))
		q.W = T(m13-m31) / s
		q.X = T(m12+m21) / s
		q.Y = 0.25 * s
		q.Z = T(m23+m32) / s
	...
	s = T(2 * stdmath.Sqrt(1+m33-m11-m22))
	q.W = T(m21-m12) / s
	q.X = T(m13+m31) / s
	q.Y = T(m23+m32) / s
	q.Z = 0.25 * s
...
func (q *Quaternion[T]) SetFromUnitVectors(vFrom, vTo Vector3[T]) *Quaternion[T] {
	...
	if r < T(EPSILON) {
		r = 0
		if stdmath.Abs(float64(vFrom.X)) > stdmath.Abs(float64(vFrom.Z)) {
			q.X = -vFrom.Y
			q.Y = vFrom.X
			q.Z = 0
			q.W = r
...
func (q *Quaternion[T]) AngleTo(qb Quaternion[T]) T {
	return T(2 * stdmath.Acos(stdmath.Abs(float64(Clamp(q.Dot(qb), -1, 1)))))
}
...
func (q *Quaternion[T]) Length() T {
	return T(stdmath.Sqrt(float64(q.X*q.X + q.Y*q.Y + q.Z*q.Z + q.W*q.W)))
}
...
func MultiplyQuaternions[T Float](a, b Quaternion[T]) *Quaternion[T] {
	q := a.Clone()
	return q.Multiply(b)
}
...
func (q *Quaternion[T]) Slerp(qb Quaternion[T], t T) *Quaternion[T] {
	...
	sinHalfTheta := stdmath.Sqrt(sqrSinHalfTheta)
	halfTheta := stdmath.Atan2(sinHalfTheta, cosHalfTheta)
	ratioA := T(stdmath.Sin(float64(1-t)*halfTheta) / sinHalfTheta)
	ratioB := T(stdmath.Sin(float64(t)*halfTheta) / sinHalfTheta)
