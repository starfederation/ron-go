...
func (c *baseCurve[T]) ComputeFrenetFrames(segments int, closed bool) map[string][]Vector3[T] {
	...
	min := 999999.0
	tx := stdmath.Abs(float64(tangents[0].X))
	ty := stdmath.Abs(float64(tangents[0].Y))
	tz := stdmath.Abs(float64(tangents[0].Z))
	if tx <= min {
		min = tx
		normal.Set(1, 0, 0)
	}
	...
	for i := 1; i <= segments; i++ {
		...
		if vec.Length() > 0.0001 {
			vec.Normalize()
			theta := stdmath.Acos(stdmath.Max(-1, stdmath.Min(1, float64(tangents[i-1].Dot(tangents[i])))))
			normals[i].ApplyMatrix4(*mat.MakeRotationAxis(*vec, T(theta)))
		}
		binormals[i] = *CrossVector3s(tangents[i], normals[i])
	}
	if closed {
		theta := T(stdmath.Acos(stdmath.Max(-1, stdmath.Min(1, float64(normals[0].Dot(normals[segments]))))))
		theta /= T(segments)
