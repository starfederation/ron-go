...
type Vector3[T Float] struct {
	X, Y, Z T
}

func NewVector3[T Float](x, y, z T) *Vector3[T] {
	return &Vector3[T]{X: x, Y: y, Z: z}
}

func NewZeroVector3[T Float]() *Vector3[T] {
	return NewVector3[T](0, 0, 0)
}

func NewOneVector3[T Float]() *Vector3[T] {
...
func AddVector3s[T Float](a, b Vector3[T]) *Vector3[T] {
...
func SubVector3s[T Float](a, b Vector3[T]) *Vector3[T] {
	return NewVector3(a.X-b.X, a.Y-b.Y, a.Z-b.Z)
}
...
func MultiplyVector3s[T Float](a, b Vector3[T]) *Vector3[T] {
	return NewVector3(a.X*b.X, a.Y*b.Y, a.Z*b.Z)
}
...
func (v *Vector3[T]) Floor() *Vector3[T] {
	v.X = T(stdmath.Floor(float64(v.X)))
	v.Y = T(stdmath.Floor(float64(v.Y)))
	v.Z = T(stdmath.Floor(float64(v.Z)))
	return v
}

func (v *Vector3[T]) Ceil() *Vector3[T] {
	v.X = T(stdmath.Ceil(float64(v.X)))
	v.Y = T(stdmath.Ceil(float64(v.Y)))
	v.Z = T(stdmath.Ceil(float64(v.Z)))
	return v
}

func (v *Vector3[T]) Round() *Vector3[T] {
	v.X = T(stdmath.Round(float64(v.X)))
	v.Y = T(stdmath.Round(float64(v.Y)))
	v.Z = T(stdmath.Round(float64(v.Z)))
	return v
}

func (v *Vector3[T]) RoundToZero() *Vector3[T] {
	v.X = T(stdmath.Trunc(float64(v.X)))
	v.Y = T(stdmath.Trunc(float64(v.Y)))
	v.Z = T(stdmath.Trunc(float64(v.Z)))
	return v
}
...
func (v *Vector3[T]) Length() T {
	return T(stdmath.Sqrt(float64(v.LengthSq())))
}
...
func LerpVector3s[T Float](v1, v2 Vector3[T], alpha T) *Vector3[T] {
	return NewVector3(
...
func CrossVector3s[T Float](a, b Vector3[T]) *Vector3[T] {
...
func (v *Vector3[T]) AngleTo(v2 Vector3[T]) T {
	denominator := stdmath.Sqrt(float64(v.LengthSq() * v2.LengthSq()))
	if denominator == 0 {
		return stdmath.Pi / 2
	}

	theta := v.Dot(v2) / T(denominator)
	return T(stdmath.Acos(float64(Clamp(theta, -1, 1))))
}

func (v *Vector3[T]) DistanceTo(v2 Vector3[T]) T {
	return T(stdmath.Sqrt(float64(v.DistanceToSquared(v2))))
}
...
func (v *Vector3[T]) ManhattanDistanceTo(v2 Vector3[T]) T {
	return T(stdmath.Abs(float64(v.X-v2.X)) + stdmath.Abs(float64(v.Y-v2.Y)) + stdmath.Abs(float64(v.Z-v2.Z)))
}
...
func (v *Vector3[T]) SetFromSphericalCoords(radius, phi, theta T) *Vector3[T] {
	...
	sinPhiRadius := stdmath.Sin(pf) * rf
	v.X = T(sinPhiRadius * stdmath.Sin(tf))
	v.Y = T(stdmath.Cos(pf) * rf)
	v.Z = T(sinPhiRadius * stdmath.Cos(tf))
	return v
}
...
func (v *Vector3[T]) SetFromCylindricalCoords(radius, theta, y T) *Vector3[T] {
	v.X = radius * T(stdmath.Sin(float64(theta)))
	v.Y = y
	v.Z = radius * T(stdmath.Cos(float64(theta)))
	return v
}
...
func (v *Vector3[T]) RandomDirection() *Vector3[T] {
	...
	t := rand.Float64() * stdmath.Pi * 2
	f := stdmath.Sqrt(1 - u*u)
	v.X = T(f * stdmath.Cos(t))
	v.Y = T(f * stdmath.Sin(t))
	v.Z = T(u)
	return v
}
