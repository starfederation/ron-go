...
func NewBox3[T Float](minVal, maxVal Vector3[T]) *Box3[T] {
	return &Box3[T]{
		Min: minVal,
		Max: maxVal,
	}
...
func (b *Box3[T]) MakeEmpty() *Box3[T] {
	minF, maxF := stdmath.MaxFloat64, -stdmath.MaxFloat64
...
func satForAxes[T Float](axes []T, v0, v1, v2, extents Vector3[T]) bool {
	testAxis := Vector3[T]{}
	...
	for i := 0; i <= len(axes)-3; i += 3 {
		...
		r := tXf*stdmath.Abs(tXf)*xf + tYf*stdmath.Abs(tYf)*yf + tZf*stdmath.Abs(tZf)*zf
