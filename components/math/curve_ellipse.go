...
type EllipseCurve[T Float] struct {
	baseCurve[T]
	aX, aY, xRadius, yRadius, aStartAngle, aEndAngle, aRotation T
	aClockwise                                                  bool
}

func NewEllipseCurve[T Float](aX, aY, xRadius, yRadius, aStartAngle, aEndAngle, aRotation T, aClockwise bool) *EllipseCurve[T] {
...
func (c *EllipseCurve[T]) Point(t T) *Vector3[T] {
	point := &Vector3[T]{}
	twoPi := T(2 * stdmath.Pi)
	deltaAngle := c.aEndAngle - c.aStartAngle
	samePoints := stdmath.Abs(float64(deltaAngle)) < EPSILON
	...
	af := float64(angle)
	x := c.aX + c.xRadius*T(stdmath.Cos(af))
	y := c.aY + c.yRadius*T(stdmath.Sin(af))

	if c.aRotation != 0 {
		rf := float64(c.aRotation)
		cos := T(stdmath.Cos(rf))
		sin := T(stdmath.Sin(rf))
		tx := x - c.aX
		ty := y - c.aY
